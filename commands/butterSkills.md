...
2. **Where to start** — surface the `start_here` block prominently, before the table. This is the most important part. Two paths:
   ...
   - **At a hackathon?** (the most common way people arrive here) Unless the hackathon is specifically about OPC / operating frameworks, default to **butterBuild** and skip the OPC layer. Only if the hackathon explicitly emphasizes OPC is butterOPC the right entry.
   - State the rule plainly: **company -> butterOPC, just building (incl. most hackathons) -> butterBuild.**
